...
export const useGetPostById = (postId: string | null) => {
  return useQuery({
    ...
    queryFn: () => (postId ? getPostById(postId) : Promise.reject("Invalid Post ID")),
    enabled: !!postId,
  });
};

export const useGetUserPosts = (userId?: string | null) => {
  return useQuery({
    queryKey: [QUERY_KEYS.GET_USER_POSTS, userId],
    queryFn: () => (userId ? getUserPosts(userId) : Promise.resolve({ total: 0, documents: [] })),
    enabled: !!userId,
  });
};
